...
void test_gradient()
{
    ...
    mag.save_png("mag_iguana");
    dir.save_png("dir_iguana");
...
void test_double_thresholding()
{
    ...
    dt.save_png("double_threshold_iguana");
...
void test_edge_tracking()
{
    ...
    edge_track.save_png("edge_track_iguana");
...
int main(int argc, char **argv)
{
    ...
    im = rgb_to_grayscale(im);
    im = smooth_image(im, 1.4);
    im.save_png("smooth_dog");
    
    pair<Image,Image> grad = compute_gradient(im);
    Image mag = grad.first;
    Image dir = grad.second;
    feature_normalize(dir);
    mag.save_png("mag_dog");
    dir.save_png("dir_dog");
    
    Image nms = non_maximum_suppression(mag, dir);
    nms.save_png("nms_dog");
    
    Image dt = double_thresholding(nms, 0.03, 0.17, 1.0, 0.25);
    dt.save_png("double_threshold_dog");
    
    Image edge_track = edge_tracking(dt, 0.25, 1.0);
    edge_track.save_png("edge_track_dog");
